...
const faqCategories = (symbol: string, threshold: string, fee: string) => [
  {
  ...
  },
  {
    ...
    items: [
      ...
      { q: 'Will shipping charges be deducted from my refund?', a: `Yes. If your order qualified for free shipping (order above ${symbol}${threshold}) and you request a return or refund, the standard shipping charge of ${symbol}${fee} will be deducted from your refund amount. For example, if your product cost ${symbol}999 and you received free shipping, your refund will be ${symbol}${parseInt(fee) > 0 ? (999 - parseInt(fee)).toString() : '999'} after deducting the shipping charges. This applies to all free-shipping orders — the actual shipping cost we incur will be adjusted from the refund.` },
  ...
  },
  {
    ...
    items: [
      { q: 'What payment methods do you accept?', a: `We accept UPI, credit/debit cards, net banking, and Cash on Delivery (COD). For COD orders, shipping charges (${symbol}${fee}) must be paid online at checkout — the remaining product amount is paid on delivery.` },
...
export default function FAQPage() {
  ...
  const faqs = faqCategories(symbol, settings.free_shipping_threshold || '999', settings.shipping_fee || '65')
