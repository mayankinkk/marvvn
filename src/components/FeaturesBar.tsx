...
export default function FeaturesBar() {
  return (
    <section className="border-y border-bonkers-gray-100 bg-bonkers-gray-50">
      <div className="container py-8 lg:py-12">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8">
          {features.map((feature) => (
            <div key={feature.title} className="text-center">
              <feature.icon className="w-8 h-8 mx-auto mb-3 text-bonkers-black" strokeWidth={1.5} />
              <h3 className="text-xs font-semibold uppercase tracking-wider mb-1">{feature.title}</h3>
              <p className="text-xs text-bonkers-gray-500">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
