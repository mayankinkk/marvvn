...
function generateInvoiceHTML(invoice: InvoiceData): string {
  ...
  const paymentLabel = invoice.paymentMethod === 'cod' ? 'Cash on Delivery' : 'Online Payment'
  const statusText = invoice.paymentStatus === 'paid' ? 'PAID' : invoice.paymentStatus === 'failed' ? 'FAILED' : 'PENDING'
  ...
  const productRows = invoice.items.map((item) => {
    ...
    return `
      <tr style="border-bottom:1px solid #e5e5e5">
        <td style="padding:10px 0"><div style="display:flex;align-items:center;gap:10px">${imgTag}<div><div style="font-size:13px;font-weight:600;color:#000">${item.title}</div><div style="font-size:10px;color:#888;margin-top:1px;text-transform:uppercase">${item.handle}</div></div></div></td>
